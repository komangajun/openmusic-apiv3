...
const { mapSongDBModel, mapGetPlaylistDBModel } = require('../../utils');

class PlaylistsService {
  constructor(collaborationService) {
    this._pool = new Pool();
    this._collaboration_service = collaborationService;
  }
  ...
  async getPlaylistById(id) {
    ...
    return result.rows.map(mapGetPlaylistDBModel)[0];
  }
  ...
  async getSongsFromPlaylist(playlistId) {
    ...
    const result = await this._pool.query(query);
    return result.rows.map(mapSongDBModel);
  }
  ...
  async verifyPlaylistOwner(id, owner) {
    ...
    if (playlist.owner !== owner) {
      throw new AuthorizationError('Anda tidak berhak menggunakan resource ini');
    }
  }
  ...
  async verifyPlaylistAccess(playlistId, userId) {
    try {
      ...
      try {
        await this._collaboration_service.verifyCollaborator(playlistId, userId);
      } catch {
        throw error;
      }
    }
